...
send_notification() {
    if >/dev/null 2>&1 command -v notify-send ; then
        if [[ "${store}" == "save" ]] ; then
            ...
            dunstify -h string:fgcolor:#D8DEE9 '      ' -i ~/.config/dunst/icons/clipboard.svg   "<span foreground='#BF616A' font_desc='Cartograph CF Italic 19'><b>Scr</b></span><span foreground='#EBCB8B' font_desc='Cartograph CF Italic 19'><b>eens</b></span><span foreground='#A3BE8C' font_desc='Cartograph CF Italic 19'><b>hot</b></span> \n<span foreground='#D8DEE9' font_desc='Cartograph CF 13'><b>Passed to clipboard!</b></span>"
        fi
    fi
}
